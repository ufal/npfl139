...
~~~
It must hold for an optimal value function in an MDP, because future decisions
...
$$\begin{aligned}
v_0(s) &← \begin{cases} 0&\textrm{if $s$ is terminal,} \\ \textrm{any~value,~we~use~}{-∞}&\textrm{otherwise;} \end{cases} \\
...
In deterministic MDP problem (i.e., when there is always just a single next
state and a single reward), searching for the optimal value function is in fact
the same as searching for the maximum weighted path in a suitable
graph:
...
the Bellman equation and assigns zero to the terminal state(s), it is optimal.
~~~
...
~~~
Such a unique $v_*$ is the _optimal value function_ because the optimal value
function must satisfy the Bellman equation and $v_*$ is the only one fulfilling
it.
...
![w=33%,f=right](value_iteration_asynchronous.svgz)
...
~~~
Compare this result to the finite-horizon case, where $B^T v = v_*$.
...
For a coin flip win probability 40%, the asynchronous value iteration proceeds as follows.
...
One of the simplest ways to achieve that is to assume _exploring starts_, where
...
~~~
A policy is called _$ε$-soft_, if
$$π(a|s) ≥ \frac{ε}{|𝓐(s)|}.$$

~~~
We call an $ε$-soft policy _$ε$-greedy_, if one action has a maximum probability of
$1-ε+\frac{ε}{|𝓐(s)|}$.

~~~
The policy improvement theorem can be proved also for the class of $ε$-soft
policies. Therefore, using $ε$-greedy policy in policy improvement step, policy
iteration has the same convergence properties.
~~~
- To prove this, note that we can embed the $ε$-soft behavior “inside” the
  environment and show equivalence.
...
~~~
- environments with extremely long episodes,
~~~
...
is the Sarsa algorithm, which after generating $S_t, A_t, R_{t+1}, S_{t+1}, A_{t+1}$
computes
$$q(S_t, A_t) ← q(S_t, A_t) + α\big(R_{t+1} + [¬\textrm{done}]⋅γ q(S_{t+1}, A_{t+1}) -q(S_t, A_t)\big).$$

~~~
![w=80%,h=center](sarsa.svgz)
