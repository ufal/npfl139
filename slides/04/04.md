...
Tile coding can be considered a generalization of state aggregation.
...
~~~
In fact, the above rule is not an SGD update, because for a general $v̂$ there does not exist
...
Recall the previously described 1000-state random walk, where transitions lead
...
talk about yet) with tile coding of 8 overlapping tiles covering position and
...
- **Frame stacking** is utilized—the input to the network are the last $4$
  ...
  input is an image with $4$ channels.
~~~
...
~~~
  - Furthermore, the gradient of the MSE loss is also clipped to the $[-1, 1]$ range. In other words,
    the loss used is in fact the $\textrm{smooth}_{L_1}$ loss (or Huber loss).
...
Because behavior policy in Q-learning is an $ε$-greedy variant of the target
