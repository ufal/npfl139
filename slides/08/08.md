...
sampled from the behaviour policy (similarly to how expected Sarsa is also an
...
![w=58%,h=center](ddpg_results.svgz)
...
gradient $∇_{→φ} q_{→θ_i}(s, a)$, we only need to reprametrize the policy as
$$a = f_{→φ}(s, ε).$$
