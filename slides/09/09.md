...
to obtain weights $→ω_i$. The read vector is then computed as $→ω_i^\T ⇉M$.
...
$⇉M ← ⇉M + →v_\textit{wr}[→e_t, 0]^\T + →v_\textit{ret}[0, →e_t]^\T$.
...
![w=60%](ctf_interface.svgz)![w=39%](ctf_interface_description.svgz)

---
class: middle
# For the Win agent for Capture The Flag

![w=44%](ctf_results_1.svgz)![w=55%](ctf_results_2.svgz)
