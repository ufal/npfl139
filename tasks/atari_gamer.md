...
`python3 -m npfl139.play.atari GAME_NAME [--zoom=4] [--frame_skip=1]`
command, so for example `python3 -m npfl139.play.atari Pong`.

The template [atari_gamer.py](https://github.com/ufal/npfl139/tree/master/labs/05/atari_gamer.py)
shows how to create the Atari environment. The `args.game` is the chosen game,
and options for frame skipping, frame stacking, grayscaling and resizing are
provided.
