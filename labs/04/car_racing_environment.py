#!/usr/bin/env python3
# Modified CarRacing-v2 environment; changes by Milan Straka <milan@strakovi.com>
# - The rendering is much faster, because we:
#   - skip the "far" objects before even transforming them (using heuristics from *-v0 env)
#   - render directly to 96x96
...
class CarRacingFS(gym.Env, EzPickle):
    """
    ## Description
    ...
    The reward is -0.1 every frame and +1000/N for every track tile visited, where N is the total number of tiles
     visited in the track. For example, if you have finished in 732 frames, your reward is 1000 - 0.1*732 = 926.8
     points.
    ...
    ```python
    >>> import gymnasium as gym
    >>> env = gym.make("CarRacing-v2", render_mode="rgb_array", lap_complete_percent=0.95, continuous=False)
    >>> env
    ...
    * `continuous=True` converts the environment to use discrete action space.
    ...
    environment. `domain_randomize` must be `True` on init for this argument to work.
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        for _ in range(self.frame_skip):
            ...
            if self.frames > 1000:
                truncated = True
            if terminated or truncated or action is None:
                break

        self.state = self._render("state_pixels")
        if self.render_mode == "human":
            self.render()
        return self.state, self.reward - self.prev_reward, terminated, truncated, {}
    ...
    def _render(self, mode: str):
        ...
        zoom = (0.1 * max(1 - self.t, 0) + min(self.t, 1)) * ZOOM * SCALE * H / WINDOW_H
        scroll_x = -(self.car.hull.position[0]) * zoom
        scroll_y = -(self.car.hull.position[1]) * zoom
        trans = pygame.math.Vector2((scroll_x, scroll_y)).rotate_rad(angle)
        trans = (W / 2 + trans[0], H / 4 + trans[1])

        self._render_road(zoom, trans, angle, self.car.hull.position)
        self.car.draw(
            self.surf,
            zoom,
            trans,
        ...
        if H not in self.fonts:
            pygame.font.init()
            self.fonts[H] = pygame.font.Font(pygame.font.get_default_font(), 42 * H // WINDOW_H)
        text = self.fonts[H].render("%04i" % self.reward, True, (255, 255, 255), (0, 0, 0))
        text_rect = text.get_rect()
        text_rect.center = (W * 2.5 / 40, H - H * 2.5 / 40.0)
    ...
    def show_state(self, state):
        ...
        if self.screen is None:
            pygame.init()
            pygame.display.init()
            self.screen = pygame.display.set_mode((STATE_W * UPSCALE, STATE_H * UPSCALE))
        if self.clock is None:
            self.clock = pygame.time.Clock()

        surface = pygame.Surface((STATE_W * UPSCALE, STATE_H * UPSCALE))
        pixels = pygame.surfarray.pixels3d(surface)
        pixels[:, :, :] = state.repeat(UPSCALE, axis=0).repeat(UPSCALE, axis=1).transpose([1, 0, 2])
        del pixels
    ...
    def _render_road(self, zoom, translation, angle, scroll):
        bounds = PLAYFIELD
        field = [
            (bounds, bounds),
            (bounds, -bounds),
        ...
        # draw grass patches
        grass = []
        mindist = (2 * GRASS_DIM ** 2 + (self.surf.get_width() / 2) ** 2 + self.surf.get_height() ** 2) / (zoom ** 2)
        for x in range(-20, 20, 2):
            if (x * PLAYFIELD / 20 - scroll[0]) ** 2 >= mindist:
                continue
            for y in range(-20, 20, 2):
                if (y * PLAYFIELD / 20 - scroll[1]) ** 2 >= mindist:
                    continue
                grass.append(
                    [
        ...
        for poly, color in self.road_poly:
            if (poly[0][0] - scroll[0]) ** 2 + (poly[0][1] - scroll[1]) ** 2 >= mindist:
                continue
    ...
    def _create_image_array(self, surface):
        return np.transpose(
...
gym.envs.register(
    id="CarRacingFS-v2",
    entry_point=CarRacingFS,
    reward_threshold=900,
)


# Can be used to make static code analysis happy when importing this module.
def register() -> None:
    pass
...
if __name__ == "__main__":
    ...
    env = CarRacingFS(args.frame_skip, render_mode=None if args.state else "human")
