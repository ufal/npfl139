...
class AZQuiz(BoardGame):
    ...
    def render_to_surface(self, surface, weights=None, weight_max=None):
        ...
        if weights is not None:
            if self._font is None:
                ...
                self._font = pygame.freetype.SysFont(None, self._A // 2)
            weight_max = weight_max or max(weights[self.valid_actions()], default=1)

        for action, (x, y) in enumerate(self._CENTERS):
            i, j = self._ACTION_X[action], self._ACTION_Y[action]
            color = self._COLORS[self._board[j, i] + 3 * (
                action == self._last_action or (self._winning_stones is not None and self._winning_stones[j, i]))]
            pygame.gfxdraw.filled_polygon(surface, [(px + x, py + y) for px, py in self._POLYGON], color)

            if weights is not None and self.valid(action) and np.isfinite(weights[action]):
                weight = weights[action] / weight_max if weight_max else 0.5
                color = (255 * (1 - weight), 192 * weight, 0, 128)
                pygame.gfxdraw.filled_polygon(surface, [(px + x, py + y) for px, py in self._POLYGON], color)
                rect = self._font.get_rect(f"{100 * weights[action]:04.1f}")
                rect.center = (x, y)
                self._font.render_to(surface, rect, None, color[:3])
    ...
    def mouse_click_to_action(self, pos):
        for action, (x, y) in enumerate(self._CENTERS):
            ...
            if distance2 < self._I * self._I and self.valid(action):
                return action
        return None
    ...
    def keyboard_input(self):
        ...
        for j in range(self.N):
            ...
            for mode in range(2):
                board[-1] += "  " * (self.N - 1 - j)
                for i in range(j + 1):
                    board[-1] += " " + (SYMBOLS[self._board[j, i]] * 2 if mode == 0 else f"{action + i:2d}") + " "
                board[-1] += "  " * (self.N - 1 - j)
            action += j + 1
        ...
        while action is None or not self.valid(action):
            try:
                action = int(input(f"Choose action for player {SYMBOLS[2 + self.to_play]}: "))
            except ValueError:
                pass
        return action
    ...
    _W, _A, _I = 13 * 40, 40, 35
    _CENTERS = [(int((i - j/2) * 2 * 0.866 * A + W/2), int((j - (N - 1)/2) * 1.5 * A + W/2))
                for (A, W, N) in [(_A, _W, N)] for j in range(N) for i in range(j + 1)]
    _POLYGON = [(0, -_I), (0.866*_I+1, -_I//2), (0.866*_I+1, _I//2), (0, _I), (-0.866*_I, _I//2), (-0.866*_I, -_I//2)]
    _COLORS = [(210,210,210), (58,58,58), (58,147,192), (254,147,17), (158,158,158), (77,195,255), (255,194,77)]
